...
namespace djv
{
	namespace shape
	{
		...
		Line::Line(int64_t xStart, int64_t yStart, int64_t xEnd, int64_t yEnd) :
			_xStart(xStart),
			_yStart(yStart),
			_xEnd(xEnd),
			_yEnd(yEnd)
		...
		Line::PixelIterator::PixelIterator(int64_t xStart, int64_t yStart, int64_t xEnd, int64_t yEnd) :
			_verticalFlip(false),
		...
		bool Line::PixelIterator::getPixel(int64_t& x, int64_t& y)
		{
			x = _x;
			y = _y;
		...
		std::unique_ptr<Shape::PixelIterator> Line::getPixelIterator() const
		{
			return std::make_unique<Line::PixelIterator>(_xStart, _yStart, _xEnd, _yEnd);
		}
	}
}
